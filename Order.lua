...
function Order:SetOperation(operation, price, quantity)
	if operation ~= "B" and operation ~= "S" then
		log.error(
			string.format(
				"SetOperation: неверная операция '%s' (ожидается 'B' или 'S')",
				tostring(operation)
			)
		)
		return
	...
	if numPrice == nil or numPrice < 0 then
		log.error(
			string.format("SetOperation: неверная цена %s (ожидается >= 0)", tostring(price))
		)
		return
	end
	...
	if numQuantity == nil or numQuantity < 0 then
		log.error(
			string.format(
				"SetOperation: неверное количество %s (ожидается >= 0)",
				tostring(quantity)
			)
		)
		return
...
function Order:SetQuantity(operation, price, quantityMax)
	...
	if price ~= nil and tonumber(price) > 0 and quantityMax ~= nil and tonumber(quantityMax) > 0 and self:IsBuy() then
		...
		if self:IsBond() then
			...
			self.Quantity =
				math.floor(tonumber(quantityMax) / tonumber(priceRub) / tonumber(self.SecurityInfo.lot_size))
		else
			self.Quantity =
				math.floor(tonumber(quantityMax) / tonumber(self.Price) / tonumber(self.SecurityInfo.lot_size))
		end
