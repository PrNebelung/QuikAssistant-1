...
ClearSecurityInfoCache()
test(
  "LoadOrdersFromFile - операция из файла без несоответствия BUY/SELL",
  function()
    resetSendOrders()
    mockCSV({ { "Gazprom", " B ", "GAZP", "100", "200.00" } })
    local orders = LoadOrdersFromFile("TEST_BuyOrders.csv")
...
ClearSecurityInfoCache()
test(
  "LoadOrdersFromFile - тикер из файла с несоответствием типа BUY/SELL",
  function()
    resetSendOrders()
    mockCSV({ { "Gazprom", " B ", "GAZP", "100", "200.00" } })
    local orders = LoadOrdersFromFile("TEST_BuyOrders.csv")
...
ClearSecurityInfoCache()
test(
  "LoadOrdersFromFile - формат с неизвестными параметрами",
  function()
    resetSendOrders()
    clearTestData()
    addTestPosition("GAZP", 100, 250.00)
