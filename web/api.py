...
LOG_PATTERN = re.compile(
    r"^(INFO|WARN|ERROR|DEBUG|TRACE)\s+(\d{2}:\d{2}:\d{2})\s+\[(\w+)\]\s+(\S+):(\d+):\s*(.*)$"
)
...
def logs():
    ...
    with open(log_file, "r", encoding="cp1251", errors="replace") as f:
        for line in f:
            ...
            if (
                search
                and search not in entry["message"].lower()
                and search not in entry["raw"].lower()
            ):
                continue
...
def parse_trade(line, instruments=None):
    ...
    if len(parts) >= 5:
        ...
        price = float(parts[3])
        is_bond = (
            facevalue >= 100 or ticker.startswith("SU") or ticker.startswith("RU000")
        )
        if is_bond:
            value = (
                qty * (price / 100) * (facevalue if facevalue > 0 else 1000) * int(lot)
            )
        else:
...
def trades():
    ...
    if group_by == "date":
        ...
        for k in grouped:
            ...
            del grouped[k]["dates"]
        grouped = dict(
            sorted(grouped.items(), key=lambda x: x[1]["value"], reverse=True)
        )
