document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadOrders() {
    ...
    try {
      ...
      document
        .querySelectorAll("#orders-table .edit-input")
        .forEach((input) => {
  ...
  exportLogBtn.addEventListener("click", () => {
    const text = logEntries
      .map((e) => `${e.date} ${e.time} [${e.level}] ${e.msg}`)
      .join("\n");
  ...
  document
    .querySelector('[data-tab="actionlog"]')
    .addEventListener("click", loadActionLog);

  async function toggleOrder(isin) {
    const broker = brokerSelect.value;
    const type = fileTypeSelect.value;
    const row = document.querySelector(`[data-isin="${isin}"]`)?.closest("tr");
    const wasEnabled = row
      ? row.classList.contains("disabled") === false
      : true;
  ...
  document.querySelectorAll("#orders-table .sortable").forEach((th) => {
    th.addEventListener("click", () => {
      ...
      document
        .querySelectorAll("#orders-table .sortable")
        .forEach((h) => (h.textContent = h.textContent.replace(/[▲▼]/g, "")));
  ...
  async function loadDashboard() {
    try {
      ...
      brokerStatsDiv.innerHTML = Object.entries(data.brokers)
        .map(
          ([broker, s]) => `
      ...
      if (data.group_by && Object.keys(data.grouped).length > 0) {
        ...
        if (data.group_by === "date") {
          tradeGroupedDiv.innerHTML = `<table class="grouped-table">
                        ...
                        <tbody>${Object.entries(data.grouped)
                          .map(
                            ([date, g]) => `
        ...
        tradesTbody.innerHTML = data.trades
          .map(
            (t) => `
                    <tr class="${t.is_bond ? "trade-bond" : "trade-stock"}">
                        <td>${t.datetime}</td>
  ...
  document.querySelectorAll("#trades-table .sortable").forEach((th) => {
    th.addEventListener("click", () => {
      ...
      document
        .querySelectorAll("#trades-table .sortable")
        .forEach((h) => (h.textContent = h.textContent.replace(/[▲▾]/g, "▾")));
  ...
  const settingsFields = [
    ...
    },
    {
      ...
      fields: [
        ...
        {
          key: "bondVolumeOrderMax",
          label: "Макс. объем облигаций",
          type: "number",
        },
        { key: "volumeOrderLimit", label: "Лимит объема", type: "number" },
        {
          key: "limitActuationOrderEdge",
          label: "Лимит edge акций %",
          type: "number",
        },
        {
          key: "limitActuationOrderBondEdge",
          label: "Лимит edge облигаций %",
          type: "number",
        },
      ],
    },
    {
      ...
      fields: [
        {
          key: "sessionMorningEnabled",
          label: "Утренняя сессия",
          type: "checkbox",
        },
        {
          key: "sessionMainEnabled",
          label: "Основная сессия",
          type: "checkbox",
        },
        {
          key: "sessionEveningEnabled",
  ...
  function renderSettingsForm() {
    ...
    settingsForm.innerHTML = settingsFields
      .map(
        (group) => `
                ...
                ${group.fields
                  .map((f) => {
                    ...
                    if (f.type === "time") {
                      const h =
                        data[f.key + "Hour"] ??
                        brokerDefaults[f.key + "Hour"] ??
                        10;
                      const m =
                        data[f.key + "Min"] ??
                        brokerDefaults[f.key + "Min"] ??
                        0;
                      const s =
                        data[f.key + "Sec"] ??
                        brokerDefaults[f.key + "Sec"] ??
                        30;
                      return `<div class="settings-row">
                            <label>${f.label}</label>
  ...
  document
    .querySelector('[data-tab="control"]')
  ...
  refreshPricesBtn.addEventListener("click", async () => {
    ...
    try {
      const res = await fetch("/api/instruments/refresh-prices", {
        method: "POST",
      });
